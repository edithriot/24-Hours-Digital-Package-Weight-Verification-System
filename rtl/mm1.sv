...
input clk,
 ...
 input pkg_detect,
  input [7:0] weight_in,
  input [7:0] min_weight,    input [7:0] max_weight,
    output sort_accept,
    output sort_reject,
    output fault,
...
package_counter counter (
    .clk(clk),
  .rst(rst),
    .pkg_detect(pkg_detect),    .accept(accept),
  .reject(reject), .total_count(total_count),
    .accept_count(accept_count),  .reject_count(reject_count)
);
// Fault Alert 
